...
class Heap:
    ...
    def __siftDownFromTo(self, fromIndex, last):
        ...
        ### WRITE YOUR CODE HERE###
        current = fromIndex
        child = self.__largestChild(current, last)
        while child is not None and self.data[current] < self.data[child]:
            self.data[current], self.data[child] = self.data[child], self.data[current]
            current = child
            child = self.__largestChild(current, last)
